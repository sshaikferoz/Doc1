...
export default function CardChartToggle(props) {
  ...
  const [clickedInd, setClickedInd] = useState(null)
  ...
  const chartSkeletonPaths = [
    '/images/ShippingPriceIndex-graph.svg',
    '/images/MaterialPriceIndex-graph.svg',
    '/images/AirFreightIndex-graph.svg',
    '/images/GlobalPressureIndex-graph.svg',
  ]
  ...
  const formatCurrency = (data) => {
    ...
    const value = dividedByThousand
      ? Number(lastItem[keyFigureKeys[0]] / 1000).toFixed(2)
    ...
    <div className={styles.wrap}>
      <div className={styles.cardsWrap}>
        {allData.map((data, ind) => {
          ...
          const value = formatCurrency(data)
            ...
            <div key={`card-${ind}`} onClick={() => setClickedInd(ind)}>
              <MarketCard>
                <div className="p-4 flex items-center gap-4">
                  <img
                    src={`${process.env.NEXT_PUBLIC_BSP_NAME}${
                      iconPathes[ind] || ''
                    }`}
                    className="w-14"
